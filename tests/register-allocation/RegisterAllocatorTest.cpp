...
TEST_F(RegisterAllocatorTest, RegisterForEachVariable) {
  ...
  for (size_t i = 0; i < registers.size(); i++) {
    ...
    EXPECT_EQ(arg->getValue<uint64_t>(), i + 1);
  }
  ...
  EXPECT_EQ(instructions[15]->getArg1()->getValue<uint64_t>(), 16);
  ...
  EXPECT_EQ(instructions[16]->getArg1()->getValue<uint64_t>(), 17);
  ...
  EXPECT_EQ(instructions[instructions.size() - 2]->getArg1()->getValue<uint64_t>(), 60);
  // syscall
