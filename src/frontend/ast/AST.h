...
namespace Wisnia {
    ...
    namespace AST {
        ...
        public:
            ...
            void addGlobalClassDef(std::unique_ptr<Root> classDef) { globalClassDefs_.push_back(std::move(classDef)); }
            void addGlobalFnDef(std::unique_ptr<Root> fnDef) { globalFnDefs_.push_back(std::move(fnDef)); }

            // Appends a child
            void addNode(std::unique_ptr<Root> child) { children_.push_back(std::move(child)); }
...
// Types
//----------------------------------------------------------------------------------------------------------------------
        // An abstract definition for TType node
...
//----------------------------------------------------------------------------------------------------------------------
        ...
        class FnCallExpr : public Expr {
            std::shared_ptr<Basic::Token> className_;  // can be nullptr if the function isn't inside a class
...
//----------------------------------------------------------------------------------------------------------------------
        ...
        public:
            explicit ConstructorDef(const std::shared_ptr<Basic::Token> &tok) : MethodDef(tok) { token_ = tok; }
        ...
        public:
            explicit DestructorDef(const std::shared_ptr<Basic::Token> &tok) : MethodDef(tok) { token_ = tok; }
        ...
        public:
            explicit ClassDef(const std::shared_ptr<Basic::Token> &tok) : Def(tok) { token_ = tok; }
