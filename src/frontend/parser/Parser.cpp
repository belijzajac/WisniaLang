...
static inline std::vector<TType> PrimTypes {
    TType::KW_VOID,
    TType::KW_INT,
    TType::KW_BOOL,
    TType::KW_FLOAT,
...
std::unique_ptr<Type> Parser::parsePrimitiveType() {
    if (std::any_of(PrimTypes.begin(), PrimTypes.end(), [&](TType t) { return peek()->getType() == t; })) {
...
std::unique_ptr<Expr> Parser::parseEqExpr() {
    ...
    while (hasAnyOf(TType::OP_EQ, TType::OP_NE)) { // ... <EQUALITY_SYMB> <COMPARE_EXPR>
...
std::unique_ptr<Expr> Parser::parseCompExpr() {
    ...
    while (hasAnyOf(TType::OP_G, TType::OP_GE, TType::OP_L, TType::OP_LE)) {  // ... <COMPARISON_SYMB> <ADD_EXPR>
        auto tokType = peek()->getType();
...
std::unique_ptr<Expr> Parser::parseAddExpr() {
    ...
    while (hasAnyOf(TType::OP_ADD, TType::OP_SUB)) { // ... <ADD_OP> <MULT_EXPR>
        auto tokType = peek()->getType();
        expect(tokType); // expect either "+" or "-"
...
std::unique_ptr<Expr> Parser::parseMultExpr() {
    ...
    while (hasAnyOf(TType::OP_MUL, TType::OP_DIV)) { // ... <MULT_OP> <UNARY_EXPR>
        auto tokType = peek()->getType();
        expect(tokType); // expect either "*" or "/"
...
std::unique_ptr<Expr> Parser::parseMethodCall() {
    ...
    // eat "." or "->"
    hasAnyOf(TType::OP_METHOD_CALL, TType::OP_FN_ARROW) ? consume() : throw ParserError{"Unknown method call symbol"};
